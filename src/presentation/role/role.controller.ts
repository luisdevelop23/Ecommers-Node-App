...
export class RoleController {
  constructor(private readonly RP: RoleRepository) {}

  public getRoles = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const role = await this.RP.getRoles();
      if (role.length === 0) {
        res.status(200).json({
  ...
  public getRole = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const role = await this.RP.getRole(id);
      if (!role) {
        res.status(200).json({
  ...
  public createRole = async (
    ...
    try {
      ...
      const role = await this.RP.createRole(data as RoleDto);
      res.status(200).json({
  ...
  public updateRole = async (
    ...
    try {
      ...
      const role = await this.RP.updateRole(id, data as RoleDto);
      res.status(200).json({
        message: "role creado correctamente",
        data: role,
        result: true,
  ...
  public deleteRole = async (
    ...
    try {
      const id = req.params.id;
      const role = await this.RP.deleteRole(id);
      res.status(200).json({
